      ...
      <div className="basis-1/2 w-full relative">
        ...
        {!isImageLoaded && (
          <div className="absolute inset-0 flex items-center justify-center bg-gray-200 animate-pulse">
            {/* Optionally, add a spinner or loading icon/text here */}
          </div>
        )}
        <img
          className="w-full"
        ...
        {error ? (
          <>
            <input
              ...
              className="w-full xs:pl-4 xs:py-3 sm:px-8 sm:py-4 border-[2px] border-solid border-red-600 outline-none focus:border-red-500"
            />
          ...
          <input
            ...
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="w-full xs:pl-4 xs:py-3 sm:px-8 sm:py-4 border border-solid border-[#AF4D32] focus:border-[2px] focus:border-[#e76241] outline-none"
          />
        )}

        <button
          onClick={handleLogin}
          className="w-full bg-[#AF4D32] mt-4 text-white xs:py-3 sm:px-8 sm:py-4 outline-none focus:border-[3px] focus:border-[#fcb6a4] focus:rounded-[4px] rounded-sm"
        >
          Login
        </button>
      </div>
